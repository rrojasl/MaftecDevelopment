    ...
    public class EmbarqueController : ApiController
    {
        ...
        public object ObtieneArea(string token)
        {
            ...
            if (tokenValido)
            {
                ...
                string patiosIN = "";
                string proyectoIDs = ConsultaBD.Instance.ObtenerRelUsuarioProyecto(usuario.UsuarioID, out patiosIN);

                return ConsultaBD.Instance.ObtenerAreas(proyectoIDs, patiosIN);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
        ...
        [HttpGet]
        public object ObtieneCuadrante(string token, int AreaID)
        {
            ...
            if (tokenValido)
            {
                ...
                return ConsultaBD.Instance.ObtenerCuadrantes(proyectoIDs, patiosIN, AreaID);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
        ...
        public object ObtieneDatosConsulta(string token, int AreaID, int CuadranteID)
        {
            ...
            if (tokenValido)
            {
                ...
                List<Sam3_Embarque_Get_Consulta_Result> result = (List<Sam3_Embarque_Get_Consulta_Result>)ConsultaBD.Instance.ObtenerDatosConsulta(proyectoIDs, patiosIN, AreaID, CuadranteID);
